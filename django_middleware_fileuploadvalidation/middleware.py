...
class FileUploadValidationMiddleware(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs) -> None | HttpResponse:
        logging.debug("[Middleware][PV] - Processing view: {}".format(view_func.__name__))

        handler = FileUploadHandler()
        config = None

        if hasattr(view_func, "_file_upload_config"):
            logging.debug("[Middleware][PV] - View with name '{}' is file upload wrapped.".format(view_func.__name__))
            config = getattr(view_func, "_file_upload_config")
            handler.set_request(request, config)
            logging.debug("[Middleware][PV] - Config:")
            pp.pprint(config)
        else:
            logging.debug("[Middleware][PV] - View with name '{}' is not file upload wrapped.".format(view_func.__name__))
        ...
        if request.method == "POST" and request.FILES:
            ...
            if request.block_request:
                logging.debug("[Middleware][PV] - Blocking request.")
